import re, requests, json, datetime, hashlib, random, math
...
class Client(object):
	...
	def get_stream_auth_by_songid(self, song_id):
		self.request('getStreamKeyFromSongIDEx', {
			'songID': song_id,
			'prefetch': False,
			'mobile': False,
			'country': COUNTRY
	...
	def get_stream_auth(self, song):
		self.get_stream_auth_by_songid(song.id)

	# Get song stream url by ID
	def get_song_url_by_id(self, id):
		resp = get_stream_auth_by_songid(id)
		return "http://" + resp['ip'] + "/stream.php?streamkey=" + resp['stream_key']
...
class Song(object):
	...
	# Presentable format
	def to_s(self):
		':'.str(self.id, self.name, self.artist)
...
print songs
client.get_stream_auth(songs[0])